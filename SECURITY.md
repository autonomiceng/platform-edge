...
The container has no Docker socket. Caddy’s admin API is loopback-only at `localhost:2019` inside the container and is not published externally. Bootstrap generates no secrets, but `edge-data` holds TLS and internal CA private keys. Protect volume backups and distribute only the exported public root certificate. See [ingress operations](docs/operations/ingress.md) for DNS, port ownership and CA export.
