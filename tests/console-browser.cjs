...
await page.route('https://test.ts.net/stack-versions/gateway', r => r.fulfill({status:versionsAvailable?200:502,contentType:'application/json',body:JSON.stringify({images:{litellm:'1.2.3'}})}));
await page.goto('https://test.ts.net/');await page.waitForFunction(()=>!document.querySelector('#refresh').disabled);
const link=page.locator('h2 a').filter({hasText:'Agent Backplane'});assert.equal(await link.getAttribute('href'),null);
...
await page.getByRole('button',{name:'Copy Agent Backplane endpoint',exact:true}).click();
assert.equal(await page.evaluate(()=>navigator.clipboard.readText()),'https://test.ts.net:8448');
await page.getByRole('status').filter({hasText:'Copied'}).waitFor();
assert.equal(await page.locator('dd[data-version]').first().textContent(),'1.2.3');
...
await page.getByRole('button',{name:'Check again',exact:true}).click();
await page.waitForFunction(()=>!document.querySelector('#refresh').disabled);
assert.equal(await page.locator('dd[data-version]').count(),0,'Missing optional metadata must clear stale versions');
assert.equal(await link.getAttribute('href'),'https://test.ts.net:8448/dashboard','Missing gateway metadata must not disable Backplane');
