...
Docker access, and does not enable lingering. `--check` replaces `--install` for a
read-only check of the user manager and unit custody, including a fresh unit destination.
An existing pair is accepted only when both owned, private, single-link regular files
match the exact generated bytes for the original checkout and env. Symlinks, foreign
selections, malformed or partial pairs, and drop-ins are refused without overwriting.
Arguments retain systemd quoting; new files and their directory are fsynced.
...
no drop-ins, enables the timer, then checks enabled and active state. A failure after
activation starts retains the pair. Repeat the same installation command to recover;
...
Backplane also receives its qualified `--compose-project`, ordered absolute
`--compose-file` arguments, and every native `--profile`. There is no aggregate unit
