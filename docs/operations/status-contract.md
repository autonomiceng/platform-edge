...
`Cache-Control: no-store`. Edge needs no Docker socket or administrative credential.
...
An observation can have a known version and unknown readiness, or vice versa.
...
Consumers ignore unknown component IDs and additive fields. They reject duplicate IDs,
wrong kinds for known IDs, invalid required fields and malformed timestamps; they never
turn malformed data into healthy state. New IDs or optional fields are additive changes.
...
Task success reports the last execution, not continuous service readiness.
...
An observation older than `validForSeconds` is stale, regardless of `generatedAt`.
Missing observation time is unknown. Timestamps over five seconds in the future are
invalid. Stale and invalid observations cannot produce a current healthy indicator.
...
component timestamp. `unsupported.json` must not be consumed as version 1. Tests should
