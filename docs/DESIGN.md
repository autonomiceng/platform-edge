...
The root `Caddyfile` holds the shared global block and issuer snippets. `routes.d/gateway.caddy`, `backplane.caddy` and `observability.caddy` own the site blocks. The fallback page is the single `docker/console/index.html` file, mounted read-only. It refreshes access settings and health at load, on request, and every 30 seconds while visible, without continuous animation. Cards link their titles to application interfaces and provide copyable endpoints. Optional `/stack-versions/gateway` metadata supplies pinned versions without forwarding browser credentials; a missing gateway leaves version rows absent. Project icons are bundled locally. The uncached `/edge-config.json` endpoint keeps already-open pages current after access setup changes.
